...
def generate_launch_description():
    ...
    shell_cmd = subprocess.Popen(
        'lsusb | grep -E "Silicon Labs CP210x UART Bridge|QinHeng Electronics" ',
        shell=True,
    ...
    alarms = ExecuteProcess(
        cmd=[["ros2 ", "launch ", "dobot_diagnostics ", "alarms_analyzer.launch.py"]],
        shell=True,
        output="log",
        condition=IfCondition(PythonExpression(valid_tool)),
    )
    ...
    homing = ExecuteProcess(
        cmd=[["ros2 ", "launch ", "dobot_homing ", "dobot_homing.launch.py"]],
        shell=True,
        output="screen",
        condition=IfCondition(PythonExpression(valid_tool)),
    )
    ...
    PTP_action = ExecuteProcess(
        cmd=[["ros2 ", "launch ", "dobot_motion ", "dobot_PTP.launch.py"]],
        shell=True,
        output="screen",
        condition=IfCondition(PythonExpression(valid_tool)),
    )
    ...
    tool_null_event = RegisterEventHandler(
        OnProcessStart(
            target_action=tool_null, on_start=[LogInfo(msg="Loading tool parameters.")]
        )
    )
    ...
    alarms_event = RegisterEventHandler(
        OnProcessStart(
            ...
            on_start=[LogInfo(msg="Starting the diagnostics module.")],
        )
    )
    ...
    gripper_event = RegisterEventHandler(
        OnProcessStart(
            ...
            on_start=[LogInfo(msg="Gripper control service started.")],
        )
    )
    ...
    suction_cup_event = RegisterEventHandler(
        OnProcessStart(
            ...
            on_start=[LogInfo(msg="Suction Cup control service started.")],
        )
    )
    ...
    on_shutdown = RegisterEventHandler(
        OnShutdown(
            on_shutdown=[
                LogInfo(
                    msg=[
                        "Dobot Magician control system launch was asked to shutdown: ",
    ...
    tool_null_sched = TimerAction(period=1.0, actions=[tool_null])

    alarms_sched = TimerAction(period=2.0, actions=[alarms])

    gripper_sched = TimerAction(period=2.0, actions=[gripper])

    suction_cup_sched = TimerAction(period=2.0, actions=[suction_cup])

    homing_sched = TimerAction(period=3.0, actions=[homing])

    trajectory_validator_sched = TimerAction(period=5.0, actions=[trajectory_validator])

    PTP_action_sched = TimerAction(period=7.0, actions=[PTP_action])

    robot_state_sched = TimerAction(period=18.0, actions=[robot_state])
